        ...
        if self.batch_processor is not None:
            ...
            for i in range(num_samples):
                ...
                for key in self.keys:
                    if key not in ['img_metas', 'img', "dpt", 'points']:
                        data[key] = data_batch[key]
                    else:
                        if key == 'img':
                            data['img'] = DataContainer(data=[data_batch['img'].data[0][:, i]], cpu_only=data_batch['img'].cpu_only, stack=True)
                        elif key == 'dpt':
                            data['dpt'] = DataContainer(data=[data_batch['dpt'].data[0][:, i]], cpu_only=data_batch['dpt'].cpu_only, stack=True)
                        elif key == 'points':
                            data['points'] = DataContainer(data=[data_batch['points'].data[0][:, i]], cpu_only=data_batch['points'].cpu_only, stack=True)
                        elif key == 'img_metas':

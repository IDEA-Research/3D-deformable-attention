...
@HEADS.register_module()
class BEVFormerHead_DFA3D(BEVFormerHead):
    
    @auto_fp16(apply_to=('mlvl_feats', 'mlvl_dpt_dist'))
    def forward(self, mlvl_feats, mlvl_dpt_dist , img_metas, prev_bev=None,  only_bev=False):
        """Forward function.
        Args:
            mlvl_feats (tuple[Tensor]): Features from the upstream
                network, each is a 5D-tensor with shape

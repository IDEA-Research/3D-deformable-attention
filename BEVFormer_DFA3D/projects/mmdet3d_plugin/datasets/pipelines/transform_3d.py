...
class RandomScaleImageMultiViewImage(object):
    ...
    def __repr__(self):
        ...
        return repr_str
@PIPELINES.register_module()
class RandomScaleImageMultiViewImageDpt(object):
    """Random scale the image
    Args:
        scales
    """
    ...
    def __call__(self, results):
        ...
        results['img'] = [mmcv.imresize(img, (x_size[idx], y_size[idx]), return_scale=False) for idx, img in
                          enumerate(results['img'])]
        if "dpt" in results.keys():
            results["dpt"] = [mmcv.imresize(img, (x_size[idx], y_size[idx]), return_scale=False, interpolation="nearest") for idx, img in enumerate(results['dpt'])]  # x y should also transform along this.

...
class CustomNuScenesDataset_(NuScenesDataset):
    r"""NuScenes Dataset.
...
@DATASETS.register_module()
class CustomNuScenesDataset(CustomNuScenesDataset_):
    def __init__(self, queue_length=4, bev_size=(200, 200), overlap_test=False, get_loss=False, *args, **kwargs):
        super().__init__(queue_length, bev_size, overlap_test, *args, **kwargs)
    ...
    def union2one(self, queue):
        ...
        if "points" in queue[0].keys():
            ...
            list_len=[len(each) for each in points_list]
            max_len=max(list_len)
            points_list=[torch.cat([each,each.new_zeros([max_len-list_len[i],5])]) for i,each in enumerate(points_list)]
            queue[-1]['points'] = DC(torch.stack(points_list),
    ...
    def get_data_info(self, index):
        ...
        if (not self.test_mode) or self.get_loss:
            annos = self.get_ann_info(index)
            input_dict['ann_info'] = annos
